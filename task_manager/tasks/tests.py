...
class TaskTests(TestCase):
    ...
    def test_task_update_view(self):
        task = Task.objects.create(name='Old Task', description='Old', status=self.status, author=self.user)
    ...
    def test_task_delete_view(self):
        task = Task.objects.create(name='Delete Task', description='To delete', status=self.status, author=self.user)
        TaskMembership.objects.create(task=task, user=self.user, role='creator')
